...
class BroccoliPug extends MultiFilter {
  constructor(inputNode, inputGlobs, options) {
    ...
    if (!Array.isArray(inputGlobs)) throw new Error('Expected string or array of input files or globs, got ' + inputGlobs)
    this.inputGlobs = inputGlobs

    this.moduleType = options.moduleType || 'es'
    if (this.moduleType !== 'es' && this.moduleType !== 'cjs' &&
        this.moduleType !== 'none') {
      throw new Error('Expected moduleType "es", "cjs" or "none", got ' + this.moduleType)
    }
  }
  ...
  build() {
    ...
    return this.buildAndCache(inputFiles, (inputFilePath, outputDirectoryPath) => {
      ...
      if (this.render) {
        ...
        let res = pug.compileClientWithDependenciesTracked(source, pugOptions)
        output = res.body
      ...
      return [inputFilePath].concat(dependencies)
